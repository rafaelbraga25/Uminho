...
public class InfoProdutos implements Serializable {
    ...
    /**
     * Devolve as unidades vendidas de Produto.
     * 
     * @return Unidades vendidas de Produto. 
     */
    public int getVendas() {
        return mapVF.values()
                    .stream()
                    .reduce(0, (s, p) -> s += p.getVendas(), 
                            (s1, s2) -> s1 + s2);
    }

    
    /**
     * Devolve o total faturado das vendas de Produto.
     * 
     * @return Faturado das vendas de Produto. 
     */
    public double getFaturado() {
        return mapVF.values()
                    .stream()
                    .reduce(0.0, (s, p) -> s += p.getFaturado(), 
                            (s1, s2) -> s1 + s2);
    }
    
    
    /**
     * Devolve um TuploIntDoubleSet com o número de unidades que de Produto
     * vendidas, o total faturado com o Produto e um Set com todos os códigos dos
     * clientes que compraram o Produto.
     * 
     * @return Número de unidades de Produto vendidas, total faturado com o 
     *         Produto e um Set com todos os códigos dos clientes que compraram
     *         o Produto.
     */
    public TuploIntDoubleSet getProdutosDistintos() {
        TuploIntDoubleSet t = new TuploIntDoubleSet();
       
        mapVF.values().forEach(v -> { 
            t.addInteiro(v.getVendas());
            t.addReal(v.getFaturado());
        });
        
        t.addSet(mapVF.keySet());
    ...
    /**
     * Para um dado Produto, devolve as unidades vendidas e o total faturado
     * com as suas vendas.
     * 
     * @param produto Produto a procurar.
     * @return Unidades vendidas e o faturado com as vendas de um Produto.
     */
    public ParVendasFaturado getVendasProduto(String produto) {
        ParVendasFaturado ret;
        
        try {
            ret = new ParVendasFaturado(mapVF.get(produto));
        }
        catch (NullPointerException e) {
            ret = new ParVendasFaturado(0, 0);
        }
