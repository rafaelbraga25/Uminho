...
public class FaturacaoGlobal implements Serializable {
    ...
    /**
     * Devolve o número de unidades vendidas num mês.
     * @param mes Mês onde calcular o número de unidades.
     * @return Unidades vendidas no mês.
     */
    public int getVendas(int mes) {
        try {
            return fatGlobal.get(mes).getVendas();
        }
    ...
    /**
     * Devolve um Map em que as Keys são meses e os Values são as unidades
     * vendidas nesse mês.
     * 
     * @return Unidades vendidas por mês.
     */
    public Map<Integer, Integer> getVendasMes() {
        Map<Integer, Integer> r = new TreeMap<>();
        
        fatGlobal.forEach((k, v) -> r.put(k, v.getVendas()));
    ...
    /**
     * Devolve o total da soma da faturação de todos os meses.
     */
    ...
    /**
     * Devolve o faturada para cada filial.
     */
    public Map<Integer, Map<Integer, Double>> getFaturadoFilial() {
       Map<Integer, Map<Integer, Double>> r = new TreeMap<>();
       
       fatGlobal.forEach((k, v) -> r.put(k, v.getFaturadoFilial()));
     ...
     */
    public int getNumNuncaComprados() {
        return nuncaComprados.size();
    }
    
    
    /**
     * Devolve um Set com os códigos dos produtos vendidos.
     */
    public Set<String> getProdsVendidos() {
        Set<String> set = new HashSet<>();
        
        fatGlobal.values().forEach(v -> {
            v.getFatFiliais().values().forEach(v1 -> {
               v1.getMapVF().keySet().forEach(c -> set.add(c));
            });
        });
    ...
    /**
     * Calcula para cada mês quantas vezes um Produto foi comprado e o total
     * faturado.
     * 
     * @param produto Código do Produto.
     * @return Quantas vezes o Produto foi comprado e o faturado, por mês.
     */
    public Map<Integer, ParVendasFaturado> getVendasProduto(String produto) {
        
        Map<Integer, ParVendasFaturado> acc = new HashMap<>();

        fatGlobal.forEach((k, v) -> {
            
            acc.put(k, v.getVendasProduto(produto));
    ...
    /**
     * Faz o Set para a Faturação Global.
     */
    public void setFatGlobal(Map<Integer, FaturacaoFiliais> map) {
        fatGlobal = this.copiaFatGlobal(map);
    }
